...
def main():
    ...
    print(f"--- 🛠️ HP 修復・再生成スクリプト (GTM ID: {GTM_ID}) 開始 ---")
    ...
    if not stub_articles:
        ...
        print("   ただし、ハブページ (index.html) のGTMタグを更新します。")
    else:
        # --- 4. AIによる優先度決定 (Top 3) ---
        print(f"\n--- [ステップ3: AIによる修復優先度の決定] ---")
    ...
    # --- 6. ハブページのGTMタグも更新 ---
    print(f"\n--- [ステップ5: ハブページのGTMタグ自動更新] ---")

    hub_paths_to_update = [
        p['file_name'] for p in all_planned_articles if p['file_name'].endswith('index.html')
    ]

    print(f"🏭 {len(hub_paths_to_update)} 件のハブページ (index.html) のGTMタグを更新します...")
    ...
    for hub_path in hub_paths_to_update:
        ...
        【最重要タスク】GTM ID ({GTM_ID}) を <head> と <body> の正しい位置に挿入してください。
